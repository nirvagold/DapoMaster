...
pub async fn jalankan_tricky_method_rust(app_handle: &AppHandle) -> Result<(), String> {
    ...
    emit_log(app_handle, "[TRICKY] Memulai proses tricky method...");
    ...
    emit_log(app_handle, "[TRICKY] Mengecek apakah aplikasi Dapodik sedang berjalan...");
    ...
    emit_log(app_handle, "[TRICKY] Dapodik tidak sedang berjalan. Lanjut ke langkah berikutnya.");
    emit_log(app_handle, "[TRICKY] --- DEBUG: tricky_method benar-benar dipanggil ---");
    emit_log(app_handle, "[TRICKY] Tricky method dimulai. Ini adalah proses 7 langkah.");

    // 1. Backup isi asli pg_hba.conf ke memory dan simpan mtime
    emit_log(app_handle, "[TRICKY] LANGKAH 1: Membackup file pg_hba.conf (ke memory, beserta mtime)...");
    ...
    if pg_hba_path.exists() {
            ...
            Ok(content) => {
                ...
                match fs::metadata(&pg_hba_path) {
                    Ok(meta) => {
                        ...
                        emit_log(app_handle, "[TRICKY] LANGKAH 1 BERHASIL: File dan mtime berhasil dibackup ke memory.");
                    },
                    Err(e) => {
                        emit_log(app_handle, &format!("[TRICKY] LANGKAH 1 PERINGATAN: Tidak bisa mendapatkan mtime: {}", e));
                    }
                }
            },
            Err(e) => {
        ...
        emit_log(app_handle, "[TRICKY] LANGKAH 1 DILEWATI: File pg_hba.conf tidak ditemukan.");
    }
    tokio::time::sleep(std::time::Duration::from_secs(1)).await;

    // 2. Modifikasi file ke trust
    emit_log(app_handle, "[TRICKY] LANGKAH 2: Memodifikasi pg_hba.conf menjadi mode 'trust'...");
    if pg_hba_path.exists() {
        ...
        emit_log(app_handle, "[TRICKY] LANGKAH 2 BERHASIL: pg_hba.conf sementara diubah ke mode 'trust'.");
        // Reload service PostgreSQL agar perubahan trust aktif
        emit_log(app_handle, "[TRICKY] LANGKAH 2b: Merestart service PostgreSQL...");
        ...
        match (output, output2) {
            (Ok(out1), Ok(out2)) if out1.status.success() && out2.status.success() => {
                emit_log(app_handle, "[TRICKY] LANGKAH 2b BERHASIL: Service PostgreSQL berhasil direstart.");
            },
            _ => {
                emit_log(app_handle, "[TRICKY] LANGKAH 2b PERINGATAN: Gagal merestart service PostgreSQL. Perubahan trust mungkin belum aktif.");
            }
        }
    } else {
        emit_log(app_handle, "[TRICKY] LANGKAH 2 DILEWATI: File pg_hba.conf tidak ditemukan.");
    }
    tokio::time::sleep(std::time::Duration::from_secs(1)).await;

    // 3. Jalankan ALTER ROLE via psql CLI dari folder Dapodik dengan timeout
    emit_log(app_handle, "[TRICKY] LANGKAH 3: Menjalankan ALTER ROLE via psql CLI dari folder Dapodik...");
    ...
    let mut child = match std::process::Command::new(psql_path)
        ...
        .spawn() {
            ...
            Err(e) => {
                ...
                emit_log(app_handle, &format!("[TRICKY] LANGKAH 3 GAGAL: Tidak bisa menjalankan psql CLI: {}", e));
                let _ = app_handle.emit("tricky_method_failed", err_msg.clone());
                std::process::exit(1);
            }
        };
    ...
    match child.wait_timeout(timeout).unwrap() {
        Some(status) if status.success() => {
            emit_log(app_handle, "[TRICKY] LANGKAH 3 BERHASIL: ALTER ROLE via psql CLI berhasil dijalankan.");
        },
        Some(status) => {
    ...
    emit_log(app_handle, "[TRICKY] LANGKAH 4: Mengembalikan file pg_hba.conf asli dari memory dan mtime...");
    if let Some(ref backup_content) = original_pg_hba {
        ...
        if let Err(e) = fs::write(&tmp_backup, backup_content) {
            ...
            if let Err(e) = restore_file_with_metadata(&tmp_backup, &pg_hba_path) {
                ...
                emit_log(app_handle, "[TRICKY] LANGKAH 4 BERHASIL: pg_hba.conf berhasil dikembalikan beserta metadata.");
            }
            let _ = fs::remove_file(&tmp_backup);
        }
    } else {
        emit_log(app_handle, "[TRICKY] LANGKAH 4 DILEWATI: Tidak ada backup di memory.");
    }
    tokio::time::sleep(std::time::Duration::from_secs(1)).await;

    emit_log(app_handle, "[TRICKY] Proses tricky method selesai.");
    Ok(())
}
...
pub async fn jalankan_tricky_method_startup() -> Result<(), String> {
    ...
    let mut child = match std::process::Command::new(psql_path)
        ...
        .spawn() {
            ...
            Err(e) => {
                let err_msg = format!("psql CLI spawn error: {}", e);
                println!("[TRICKY] LANGKAH 3 GAGAL: Tidak bisa menjalankan psql CLI: {}", e);
                std::process::exit(1);
            }
        };
    let timeout = std::time::Duration::from_secs(10);
